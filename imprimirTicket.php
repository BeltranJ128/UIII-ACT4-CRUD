...
$sentencia = $base_de_datos->prepare("SELECT id, fecha, total FROM mantenimiento WHERE id = ?");
...
$sentenciaProductos = $base_de_datos->prepare("SELECT p.nombre, p.descripcion,p.precio, pv.cantidad
FROM productos p
INNER JOIN 
cant_producto pv
ON p.id = pv.id_producto
WHERE pv.id_mantenimiento = ?");
